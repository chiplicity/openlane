...
layer1=$(grep "Layer 1 usage percentage:" $fr_log -s | tail -1 | sed -E 's/.*Layer 1 usage percentage: (\S+)%/\1/')
if ! [[ $layer1 ]]; then layer1=-1; fi
layer2=$(grep "Layer 2 usage percentage:" $fr_log -s | tail -1 | sed -E 's/.*Layer 2 usage percentage: (\S+)%/\1/')
if ! [[ $layer2 ]]; then layer2=-1; fi
layer3=$(grep "Layer 3 usage percentage:" $fr_log -s | tail -1 | sed -E 's/.*Layer 3 usage percentage: (\S+)%/\1/')
if ! [[ $layer3 ]]; then layer3=-1; fi
layer4=$(grep "Layer 4 usage percentage:" $fr_log -s | tail -1 | sed -E 's/.*Layer 4 usage percentage: (\S+)%/\1/')
if ! [[ $layer4 ]]; then layer4=-1; fi
layer5=$(grep "Layer 5 usage percentage:" $fr_log -s | tail -1 | sed -E 's/.*Layer 5 usage percentage: (\S+)%/\1/')
if ! [[ $layer5 ]]; then layer5=-1; fi
layer6=$(grep "Layer 6 usage percentage:" $fr_log -s | tail -1 | sed -E 's/.*Layer 6 usage percentage: (\S+)%/\1/')
...
physical_cells=$(echo "$endcaps + $tapcells + $diodes" | bc);#$(((endcaps+tapcells)+diodes));
